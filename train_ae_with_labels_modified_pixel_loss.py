...
def modify_reconstruction(reconstruction, nbr_of_attributes):
    ...
    for i in range(batch_size):
        ...
        if reconstruction[i][attribute_to_modify] > 0:
...
for epoch in range(start_epoch, end_epoch):
    ...
    for w, a in iterate_batches(all_w_tmp, all_a_tmp, batch_size_train):
        ...
        # modified_images = generate_images_in_w_space(modified_reconstruction, w.shape[0])
    ...
    # test loop
    del input_images, output_images, modified_images, reconstruction, modified_reconstruction, code, w, a, all_w_tmp, all_a_tmp, \
    ...
    if (epoch + 1) % 1 == 0:
        ...
        save_image(modified_images, f'ae/data/output_images/modified_epoch{epoch + 1}_{dt_string}')
        save_model(f'ae_models/model_e{epoch + 1}_{dt_string}.pch', model, optimizer)

    # increase lambda_a
    if linear_scale[epoch + 1] <= 2.0:
        lambda_pixel_loss += 0.05
        print(f'Increased lambda_a: {lambda_pixel_loss}')

    del input_images, output_images, modified_images, modified_reconstruction, reconstruction, code, w, a, all_test_a, all_test_w, \
        running_loss_a, running_loss_w, running_loss, loss_w, loss_a, loss, count_batches, running_loss_pixel_loss
    gc.collect()
    torch.cuda.empty_cache()
